			...
			<jsp:include page="../include/faq_middle.jsp" />
				...
				<div class = "page_box">
					<c:if test="${page > block }">
						<a href = "faq_list.do?page=1"> << </a>
						<a href = "faq_list.do?page=${startBlock-1 }"> < </a>
					</c:if>
					
					<c:forEach begin = "${startBlock }" end = "${endBlock }" step="1" var = "i">
						<c:if test="${i == page }">
							<b><a href = "faq_list.do?page=${i }">[${i }]</a></b>
						</c:if>
						<c:if test="${i != page }">
							<a href = "faq_list.do?page=${i }">[${i }]</a>
						</c:if>
					</c:forEach>
					
					<c:if test="${endBlock < allPage }">
						<a href = "faq_list.do?page=${endBlock + 1 }"> > </a>
						<a href = "faq_list.do?page=${allPage }"> >> </a>
					</c:if>	
				</div>
			</div>	
		</div>

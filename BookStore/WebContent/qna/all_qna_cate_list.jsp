				...
				<table class = "book_tb">
								...
								<td class = "qna_tit_td">
									...
									<c:if test="${qna.getAq_lock() == '1' }">
										<c:if test="${session_mem_id == 'admin' }">
											<a href = "${pageContext.request.contextPath }/all_qna_cont.do?num=${qna.getAq_num() }&page=${page }&cate=${cate }">${qna.getAq_title() }</a>
											<img id = "lock_img" src="img/lock.png">
										</c:if>
										<c:if test="${session_mem_id != 'admin' }">
											<a href = "${pageContext.request.contextPath }/all_qna_check_form.do?num=${qna.getAq_num() }&page=${page }&cate=${cate }&btn=cont">${qna.getAq_title() }</a>
											<img id = "lock_img" src="img/lock.png">
										</c:if>
									</c:if>
					...
					<div class = "page_box">
						<c:if test="${page > block }">
							<a href = "qna_list_cate.do?page=1&cate=${cate }"> << </a>
							<a href = "qna_list_cate.do?page=${startBlock-1 }&cate=${cate }"> < </a>
						</c:if>
						
						<c:forEach begin = "${startBlock }" end = "${endBlock }" step="1" var = "i">
							<c:if test="${i == page }">
								<b><a href = "qna_list_cate.do?page=${i }&cate=${cate }">[${i }]</a></b>
							</c:if>
							<c:if test="${i != page }">
								<a href = "qna_list_cate.do?page=${i }&cate=${cate }">[${i }]</a>
							</c:if>
						</c:forEach>
						
						<c:if test="${endBlock < allPage }">
							<a href = "qna_list_cate.do?page=${endBlock + 1 }&cate=${cate }"> > </a>
							<a href = "qna_list_cate.do?page=${allPage }&cate=${cate }"> >> </a>
						</c:if>	
					</div>	

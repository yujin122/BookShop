		...
		if(mem_id != ""){ // 로그인
			if($("#nickname").val() != ""){	// 회원글
				if(mem_id == $("#writer").val()){	//작성자
					...
					if(cate == ""){		// 전체리스트
						$("#update_btn").attr("onclick", "location = '${pageContext.request.contextPath }/all_qna_update_form.do?num="+num+"&page=${page }'");
						$("#list_btn").attr("onclick", "location = '${pageContext.request.contextPath }/qna_list.do?page=${page }'");
						
					}else{	// 메뉴리스트
						$("#update_btn").attr("onclick", "location = '${pageContext.request.contextPath }/all_qna_update_form.do?num="+num+"&page=${page }&cate=${cate }'");
						$("#list_btn").attr("onclick", "location = '${pageContext.request.contextPath }/qna_list_cate.do?page=${page }&cate=${cate }'");
					}	
				}else{		// 작성자 X
					$("#delete_btn").hide();
					if(cate == ""){		// 전체리스트
			...
			if($("#nickname").val() != ""){	// 회원글
				...
				$("#delete_btn").attr("onclick", "location = 'all_qna_check_form.do?num="+num+"&page=${page }&btn=delete'");
				if(cate == ""){		// 전체리스트
					$("#update_btn").attr("onclick", "location = 'all_qna_check_form.do?num="+num+"&page=${page }&btn=non_update'");
					$("#list_btn").attr("onclick", "location = '${pageContext.request.contextPath }/qna_list.do?page=${page }'");
				}else{	// 메뉴리스트
					$("#update_btn").attr("onclick", "location = 'all_qna_check_form.do?num="+num+"&page=${page }&cate=${cate }&btn=non_update'");
					$("#list_btn").attr("onclick", "location = '${pageContext.request.contextPath }/qna_list_cate.do?page=${page }&cate=${cate }'");
				}
			}
		} 
		...
		<jsp:include page="../include/help_side.jsp" />
			...
			<div class = "main_box">
				<div id="qna_cont_box">
					...
					<span>${qna.getAq_date().substring(0,10) }</span> <span>조회수 : ${qna.getAq_hit() }</span>
					<hr>

		...
		<div class="main_box" align="center">
		 <table>
			<tr class="top_cate_tr"> 
				<td class="top_cate">
						   <%-- [전체] 카테고리 클릭 시 보여지는 중분류 카테고리 --%>
							<c:set var="a_list" value="${cateAllList }" />
										...
										<tr>
											<c:forEach items="${a_list }" var="a_dto">
												<c:set var="cnt" value="${cnt + 1 }" />
													<td>
													<a href="category_list_product.do?code=${a_dto.getCate_code() }&menu=all">  ${a_dto.getCate_name() }</a>
													</td>
													<c:if test="${cnt%6 == 0 }">
													</tr>
													<tr>
							...
							<%-- [전체]를 제외한 나머지 대분류 카테고리 클릭 시 보여지는 중분류 카테고리 --%>	
							<c:set var="c_list" value="${cateList }" />
							<c:if test="${!empty c_list }">
								<c:forEach items="${c_list }" var="c_dto">
									<c:set var = "code" value = "${c_dto.getCate_code() }" />
						...
						<tr>
							...
							<c:if test="${empty p_dto.getB_image() }"> <!-- 도서 정보 이미지가 없을 때  -->
								<c:if test="${!empty p_dto.getS_image() }"> <!-- 판매자가 올린 이미지가 있을때 대체 -->
									<img src="${pageContext.request.contextPath }/upload/book${p_dto.getS_image() } " width="100" height="auto">
								</c:if>
								<c:if test="${empty p_dto.getS_image() }">	<!-- 판매자가 올린 이미지가 없을때 대체 -->
									<img src="./img/nophoto.png" width="100" height="auto">
								</c:if>
							</c:if>
							<c:if test="${!empty p_dto.getB_image() }">
								<img src="${p_dto.getB_image() }" width="100" height="auto">
							</c:if>	
							</td>
							<td class="state">[${p_dto.getS_state() }]</td>
							<td class="hit" rowspan="4" valign="top">조회수 : ${p_dto.getS_hit() }</td>
							<td class="nickname" rowspan="3">${p_dto.getMem_nickname() }(${p_dto.getS_member() })</td>
						</tr>
						<tr>
							<td class="name"><a href = "${pageContext.request.contextPath }/book_cont.do?s_num=${p_dto.getS_num() }&page=${page }&code=${cate_code }&menu=${menu }">${p_dto.getB_name() }</a></td>
						</tr>
						<tr>
			...
			<div class="page_box">
				<c:if test="${page > block }">
					<a href="category_list_product.do?code=${cate_code }&menu=${menu }&page=1"> << </a>
					<a href="category_list_product.do?code=${cate_code }&menu=${menu }&page=${startBlock - 1 }"> < </a>
				</c:if>

				<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
					<c:if test="${i == page }">
						<b><a href="category_list_product.do?code=${cate_code }&menu=${menu }&page=${i }">[${i }]</a></b>
					</c:if>

					<c:if test="${i != page }">
						<a href="category_list_product.do?code=${cate_code }&menu=${menu }&page=${i }">[${i }]</a>
					</c:if>
				</c:forEach>

				<c:if test="${endBlock < allPage }">
					<a href="category_list_product.do?code=${cate_code }&menu=${menu }&page=${endBlock + 1 }"> > </a>
					<a href="category_list_product.do?code=${cate_code }&menu=${menu }&page=${allPage }"> >> </a>
				</c:if>
			</div>
